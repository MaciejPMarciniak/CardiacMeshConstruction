...
class BuildXML:
    ...
    def __init__(self, source="data", output=None, key_word=None, list_of_elements=None):
        ...
        self.list_of_elements = (
            self._list_of_elements_24 if list_of_elements is None else list_of_elements
        )
    ...
    def find_ids(self):
        ...
        for filename in self.files:
            file_id = re.findall(
                r"\d+$", filename
            )  # Takes all numbers from the filename, if $ ommitted
            list_of_ids.append(
                "".join(file_id),
            )
        return list_of_ids
...
class ModelAtlas(BuildXML):
    ...
    def __init__(
        ...
        self.template_path = (
            source if template_path is None else os.path.join(self.__location__, template_path)
        )
        ...
        if not list_of_elements:
            self.files = glob.glob(
                os.path.join(self.template_path, "*{}*.vtk".format(self.prototype_id))
            )
            if not self.files:
            ...
            for elem in list_of_elements:
                self.elem = elem
                element_files = glob.glob(
                    os.path.join(
                        self.template_path, "*{}*_{}.vtk".format(self.prototype_id, self.elem)
                    )
                )
...
class ModelShooting(BuildXML):
    ...
    def __init__(
        ...
        if not list_of_elements:
            ...
            for elem in list_of_elements:
                self.elem = elem
                element_files = glob.glob(
                    os.path.join(self.template_path, "*_{}.vtk".format(self.elem))
                )
                self.files.extend(element_files)
        self.files.sort()
        self.number_of_cases = len(self.files)
    ...
    def build_with_lxml_tree(self):
        ...
        initial_control_points.text = os.path.join(
            self.template_path, "DeterministicAtlas__EstimatedParameters__ControlPoints.txt"
        )
...
def prepare_data_for_atlas_construction(
    source_path=os.path.join(str(Path.home()), "Python", "data", "h_case"),
    output_path="/home/mat/Deformetrica/deterministic_atlas_ct",
    key_word="h_case",
):
    ds = DataSet(source=os.path.join(source_path), key_word=key_word)
...
def prep_modes_for_reconstruction(
    ...
    os.rename(
        os.path.join(output_path, "model.xml"), os.path.join(output_path, "model_shooting.xml")
    )


def prep_random_cohort_for_reconstruction(
    source_path=os.path.join(
        str(Path.home()),
        "Deformetrica",
        "deterministic_atlas_ct",
    ...
    for i in range(1, 41):
        ...
        os.rename(
            ...
            os.path.join(output_path, "model_shooting_{}.xml".format(i)),
        )


def prep_predefined_cohort_for_reconstruction(
    source_path=os.path.join("DataInput", "PCA"),
    ...
    os.rename(
        os.path.join(output_path, "model.xml"),
        os.path.join(output_path, "predef_model_shooting.xml"),
    )
