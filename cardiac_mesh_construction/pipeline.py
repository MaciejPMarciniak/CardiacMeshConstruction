...
def perform_deformation(
    bash_file_path=os.path.join("DataInput", "Deformetrica", "Model"),
    bash_file="build_mesh_surfaces.sh",
):
    subprocess.call(os.path.join(bash_file_path, bash_file))


def write_meshing_file(
    gmsh_exe_path,
...
def merged_shapes_generation(
    ...
    for j in range(id_from, id_to):
        sup = MeshTetrahedralization(
            main_path=os.path.join("DataInput", "Gmsh"),
