    ...
    def build_momenta_matrix(self):
        ...
        for model_number in range(self.n_models):
            reshaped_momenta_matrix[model_number*self.n_control_points:(model_number+1)*self.n_control_points, :] = \
                self.momenta_matrix[model_number].reshape((self.n_control_points, self.n_dimensions), order='F')
...
def transform_momenta_before_pca(path_to_momenta=os.path.join(str(Path.home()), 'Deformetrica',
...
def transform_momenta_after_pca(path_to_momenta=os.path.join('DataOutput', 'PCA', 'Extremes'),
                                ):

    momenta = HandleMomenta(momenta_file_path=path_to_momenta,
...
def generate_randomly_weighted_modes(path_to_momenta=os.path.join('DataOutput', 'PCA', 'RandomMeshGeneration')):
    for i in range(1, 41):
